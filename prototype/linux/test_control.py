    ...
    def test_lock_unlock_and_agent_denial(self):
        try:
            ...
            denied = subprocess.run(
                ["runuser", "-u", "nobody", "--", str(HERE / "aishieldctl"),
                 "unlock", str(self.secret)], text=True, capture_output=True,
            )
            self.assertNotEqual(denied.returncode, 0)

            launched = subprocess.run(
                ["runuser", "-u", "nobody", "--", str(HERE / "aishield-proto"),
                 "--read", str(self.base), "--", "/bin/true"],
                text=True, capture_output=True,
            )
            self.assertNotEqual(launched.returncode, 0)
            self.assertIn(str(self.secret), launched.stderr)
            self.assertIn("grant narrower roots", launched.stderr)
        finally:
